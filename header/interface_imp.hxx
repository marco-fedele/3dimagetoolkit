...
void im3d::interface<S>::get_coordinates (uint & i, uint & j, uint & k)
{
	...
	if ( static_cast<int>(i)>=dims[0] )	i = dims[0]-1;
	if ( static_cast<int>(j)>=dims[1] )	j = dims[1]-1;
	if ( static_cast<int>(k)>=dims[2] )	k = dims[2]-1;
										  ...
										  uint & i2, uint & j2, uint & k2)
{
	...
	if ( static_cast<int>(i1)>=dims[0] )	i1 = dims[0]-1;
	if ( static_cast<int>(j1)>=dims[1] )	j1 = dims[1]-1;
	if ( static_cast<int>(k1)>=dims[2] )	k1 = dims[2]-1;
	
	if ( static_cast<int>(i2)>=dims[0] )	i2 = dims[0]-1;
	if ( static_cast<int>(j2)>=dims[1] )	j2 = dims[1]-1;
	if ( static_cast<int>(k2)>=dims[2] )	k2 = dims[2]-1;
