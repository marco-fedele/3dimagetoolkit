											...
											double threshold, bool full_connected, bool binary_output) const
{
	...
	if(I==0) ++I;
	if(J==0) ++J;
	if(I>=this->dimx-1) I = this->dimx-2;
	if(J>=this->dimy-1) J = this->dimy-2;
	if(this->dimz==1)
		K=0;
	else
	{
		if(K==0) ++K;
		if(K>=this->dimz-1) K = this->dimz-2;
	}
...
#pragma omp parallel for
		...
		for (int i = r; i < X; ++i)
			for (int j = r; j < Y; ++j)
			{
				...
				aux=0;
				if(mask[1]!=mask[0])	++aux;
				if(mask[2]!=mask[1])	++aux;
				if(mask[3]!=mask[2])	++aux;
				if(mask[4]!=mask[3])	++aux;
				if(mask[5]!=mask[4])	++aux;
				if(mask[6]!=mask[5])	++aux;
				if(mask[7]!=mask[6])	++aux;
				if(mask[7]!=mask[0])	++aux;
