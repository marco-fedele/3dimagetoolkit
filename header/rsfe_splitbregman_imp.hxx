...
void segm::rsfe_splitbregman<T>::set_param_from_getpot (std::string const & section)
{
	...
	if(itemp != -1){this->set_verbosity( static_cast<bool>(itemp) ); }
	
	itemp = ifile("maxiter", -1);
	if(itemp > 0){this->set_maxiter(itemp);}
	
	itemp = ifile("showfrequency", -1);
	if(itemp >= 0){this->set_showfrequency(itemp);}
	
	itemp = ifile("dumpfrequency", -1);
	if(itemp >= 0){this->set_dumpfrequency(itemp);}
	
	stemp = ifile("outputname", "");
	if(stemp.size()!=0 ){this->set_outputname(stemp);}
	
	itemp = ifile("save_current", -1);
	if(itemp != -1){ this->save_current = static_cast<bool>(itemp); }
	
	itemp = ifile("end_now", -1);
	if(itemp != -1){ this->end_now = static_cast<bool>(itemp); }
	
	// RSFE Parameters
	ttemp = ifile("sigma", -1.);
	if(ttemp != -1.){this->set_sigma(ttemp);}
	
	ttemp = ifile("lamda", -1.);
	if(ttemp != -1.){this->set_lamda(ttemp);}
	
	ttemp = ifile("lamda1", -1.);
	if(ttemp != -1.){this->set_lamda1(ttemp);}
	
	ttemp = ifile("lamda2", -1.);
	if(ttemp != -1.){this->set_lamda2(ttemp);}
	
	ttemp = ifile("nu", -1.);
	if(ttemp != -1.){this->set_nu(ttemp);}
	
	ttemp = ifile("beta", -1.);
	if(ttemp != -1.){this->set_beta(ttemp);}
	
	ttemp = ifile("dt", -1.);
	if(ttemp != -1.){this->set_dt(ttemp);}
	
	itemp = ifile("ls_steps", -1);
	if(itemp != -1){this->set_ls_steps( static_cast<uint>(itemp) );}
	
	ttemp = ifile("epsilon", -1.);
	if(ttemp != -1.){this->set_epsilon(ttemp);}
	
	ttemp = ifile("a0", 2.1e-6);
	if(ttemp != 2.1e-6){this->set_a0(ttemp);}
	
	ttemp = ifile("b0", -2.1e-6);
	if(ttemp != -2.1e-6){this->set_b0(ttemp);}
	
	// Connected Component
	itemp = ifile("auto_extract_conn_comp", -1);
	if(itemp != -1){this->set_auto_extract_conn_comp(itemp);}
	
	itemp = ifile("cc_binary_output", -1);
	if(itemp != -1){this->set_cc_binary_output(itemp);}
	
	itemp = ifile("cc_init_variables", -1);
	if(itemp != -1){this->set_cc_init_variables(itemp);}
	
	ttemp = ifile("cc_init_pixel_x", -1.);
	if(ttemp != -1.){this->set_cc_init_pixel_x(ttemp);}
	
	ttemp = ifile("cc_init_pixel_y", -1.);
	if(ttemp != -1.){this->set_cc_init_pixel_y(ttemp);}
	
	ttemp = ifile("cc_init_pixel_z", -1.);
	if(ttemp != -1.){this->set_cc_init_pixel_z(ttemp);}
	
	itemp = ifile("auto_extract_conn_comp_freq", -1);
	if(itemp >= 0){this->set_auto_extract_conn_comp_freq(itemp);}
	
	// Expert
	itemp = ifile("bc", -1);
	if(itemp != -1){this->set_bc( static_cast<bc_type>(itemp) );}
	
	ttemp = ifile("ls_tol", -1.);
	if(ttemp != -1.){this->set_ls_tol(ttemp);}
	
	itemp = ifile("edge_detector", -1);
	if(itemp != -1){this->set_edge_detector( static_cast<ed_type>(itemp) );}
	
	ttemp = ifile("edge_detector_sigma", -1.);
	if(ttemp != -1.){this->set_edge_detector_sigma(ttemp);}
	
	itemp = ifile("gaussian_pixel_approach", -1);
	if(itemp != -1){this->set_gaussian_pixel_approach(itemp);}
	
	ttemp = ifile("tol", -1.);
	if(ttemp != -1.){this->set_tol(ttemp);}
	
	ttemp = ifile("alpha", 1.e-9);
	if(ttemp != 1.e-9){this->set_alpha(ttemp);}
...
void segm::rsfe_splitbregman<T>::initialize_contour_as_cube(im3d::image3d<T> const & image)
{
	...
	if ( this->ix < 0 || this->ix >= X )  this->ix = 0;
	if ( this->iy < 0 || this->iy >= Y )  this->iy = 0;
	if ( this->iz < 0 || this->iz >= Z )  this->iz = 0;
	
	if ( this->fx < 0 || this->fx >= X )  this->fx = image.getdimx()-1;
	if ( this->fy < 0 || this->fy >= Y )  this->fy = image.getdimy()-1;
	if ( this->fz < 0 || this->fz >= Z )  this->fz = image.getdimz()-1;
...
void segm::rsfe_splitbregman<T>::apply (im3d::image3d<T> const & myim, im3d::image3d<T> const & init)
{
	...
	for (t=1; tollerance>tol && t<=this->maxiter && end_now==false; ++t)
	{
		...
		*/
		...
		f1*=f1; f1*=this->lamda1; // note: *= limits memory usage
		f2*=f2; f2*=this->lamda2;
		f1-=f2;
		...
		aux*=myim; aux*=myim; aux*=kones;
		kr2-=aux; kr2-=kr1; kr2/=this->lamda; kr2-=divdb;
	...
	this->ix=-1; this->iy=-1; this->iz=-1;
	this->fx=-1; this->fy=-1; this->fz=-1;
